...
export const getWallet = async (privateKey?: string) => {
  ...
  // inMemory network use rich account
  if (network.chainId.toString() === '260') {
...
export const deployContractWithArtifact = async (
  artifact: ZkSyncArtifact,
  constructorArguments?: any[],
  options?: DeployContractOptions,
  deployer?: Deployer,
) => {
  const wallet = options?.wallet ?? (await getWallet())
  if (!deployer) {
...
export const deployContract = async (
  ...
  const contract = await deployContractWithArtifact(artifact, constructorArguments, options, deployer);
